...
function filter_woocommerce_loop_add_to_cart_args( $args, $product ) {
	...
	if ( AnimatedParakeet\action__display_options( $optionvalues['display'] ) ) {
...
function render__product( $product_id, $optionvalues, $quantity = 1 ) {
	...
	$html  = '<div id="animated-parakeed-product-notice" class="' . $position . ' active">'; // Open .product-notice-wrapper.
	$html .= '<span id="appn-closer" class="dashicons dashicons-no"></span>';
	$html .= '<h5>' . esc_html( 'Added To Cart' ) . '</h5>';
